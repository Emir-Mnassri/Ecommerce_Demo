...
router.get("/admin/stats", async (req, res): Promise<void> => {
  try {
    ...
    console.error({ err }, "Admin stats error");
...
router.get("/admin/orders", async (req, res): Promise<void> => {
  try {
    ...
    console.error({ err }, "Admin orders error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.get("/admin/products", requireAdmin, async (req, res): Promise<void> => {
  try {
    ...
    console.error({ err }, "Admin products error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.post("/admin/products", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    ...
    const slug =
      d.slug ??
      d.name.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "") + "-" + Date.now();
    ...
    console.error({ err }, "Admin create product error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.put("/admin/products/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (d.discountPrice !== undefined)
      update.discountPrice = d.discountPrice != null ? d.discountPrice.toFixed(3) : null;
    ...
    console.error({ err }, "Admin update product error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.patch("/admin/products/:id/stock", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    ...
    console.error({ err }, "Admin patch stock error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.delete("/admin/products/:id", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    ...
    console.error({ err }, "Admin delete product error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
...
router.put("/admin/orders/:id/status", async (req, res): Promise<void> => {
  ...
  try {
    ...
    console.error({ err }, "Admin update order status error");
    res.status(500).json({ error: "Erreur interne" });
  }
});
